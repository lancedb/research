...
def optimized_search_pipeline(
    ...
    wandb_project: str = "squad-embedding-tuning-bge-2",
...
if __name__ == "__main__":
    ...
    tbl_name = f"squad_bge_base"
    db = lancedb.connect("db")
    if tbl_name not in db:
        print("creating new table")
        ingest(0, 87600, table_name=tbl_name, model="BAAI/bge-base-en-v1.5")
        tbl = db[tbl_name]
    ...
    optimized_search_pipeline(
                          60000, 2000,  
                          wandb_run_name=f"baseline-5",
                           embedding_model_name=f"BAAI/bge-base-en-v1.5",
    ...
    for epoch in epochs:
        ...
        tbl_name = f"squad_{epoch}_new"
        db = lancedb.connect("db")
        if tbl_name not in db:
            print("creating new table")
            ingest(0, 87600, table_name=tbl_name, model=f"ayushexel/emb-bge-base-en-v1.5-squad-{epoch}-epochs")
            tbl = db[tbl_name]
            tbl.create_fts_index("context", use_tantivy=False)
            tbl.create_index(index_type="IVF_HNSW_SQ", vector_column_name="embedding")
        tbl = db[tbl_name]
        ...
        optimized_search_pipeline(
                              60000, 2000,
                              wandb_run_name=f"emb-bge-base-en-v1.5-{epoch}-epochs-5",
                               embedding_model_name=f"ayushexel/emb-bge-base-en-v1.5-squad-{epoch}-epochs",
                              query_types = ["vector", "fts"],
                              k_values = [5],
                            table_name=tbl_name
                             )
        optimized_search_pipeline(
                              60000, 2000,
                              wandb_run_name=f"emb-bge-base-en-v1.5-{epoch}-epochs-10",
                               embedding_model_name=f"ayushexel/emb-bge-base-en-v1.5-squad-{epoch}-epochs",
                              query_types = ["vector", "fts"],
                              k_values = [10],
                            table_name=tbl_name
