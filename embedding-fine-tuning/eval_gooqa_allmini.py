...
def optimized_search_pipeline(
    ...
    wandb_project: str = "gooaq-embedding-tuning-2",
...
if __name__ == "__main__":
    tbl_name = f"gooaqnew_base"
    db = lancedb.connect("db")
    if tbl_name not in db:
        print("creating new table")
        ingest(200_000_0, 100_0000, table_name=tbl_name, model="all-MiniLM-L6-v2")
        tbl = db[tbl_name]
    ...
    optimized_search_pipeline(
                          ...
                          wandb_run_name=f"baseline-5",
                           embedding_model_name=f"all-MiniLM-L6-v2",
    ...
    for epoch in epochs:
        ...
        if tbl_name not in db:
            print("creating new table")
            ingest(200_000_0, 100_0000, table_name=tbl_name, model=f"ayushexel/emb-all-MiniLM-L6-v2-gooaq-{epoch}-epochs")
            tbl = db[tbl_name]
            tbl.create_fts_index("answer", use_tantivy=False)
            tbl.create_index(index_type="IVF_HNSW_SQ", vector_column_name="embedding")
        tbl = db[tbl_name]
        ...
        optimized_search_pipeline(
                              ...
                              wandb_run_name=f"emb-all-MiniLM-{epoch}-epochs-5",
                               embedding_model_name=f"ayushexel/emb-all-MiniLM-L6-v2-gooaq-{epoch}-epochs",
                              query_types = ["vector", "fts"],
                              k_values = [5],
                            table_name=tbl_name
                             )
