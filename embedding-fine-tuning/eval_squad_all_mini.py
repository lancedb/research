...
def ingest(
    ...
    try:
        ...
        for i in tqdm(range(0, num_examples, batch_size), desc="Ingesting Batches"):
            ...
            answers = current_batch_data['context']
            ...
            data_to_add = [
                {"context": answer, "embedding": embedding}
...
def single_query(
    ...
    retrieved_filenames = [result['context'] for result in results]
...
def optimized_search_pipeline(
    ...
    wandb_project: str = "squad-embedding-tuning-2",
    ...
    try:
        gooaq_dataset = load_dataset("rajpurkar/squad", split="train", trust_remote_code=True).skip(skip).select(range(select))
    except Exception as e:
    ...
    for example in gooaq_dataset:
        if example.get('question') and example.get('context'):
            test_data.append((example['question'], example['context']))
...
if __name__ == "__main__":
    ...
    tbl_name = f"squad_base_2"
    db = lancedb.connect("db")
    if tbl_name not in db:
        print("creating new table")
        ingest(0, 87600, table_name=tbl_name, model="sentence-transformers/all-MiniLM-L6-v2")
        tbl = db[tbl_name]
        tbl.create_fts_index("context", use_tantivy=False)
    ...
    optimized_search_pipeline(
                          60000, 2000,  
                          wandb_run_name=f"baseline-5-2",
                           embedding_model_name=f"sentence-transformers/all-MiniLM-L6-v2",
                          query_types = ["vector", "fts"],
                          k_values = [5],
                        table_name=tbl_name
                         )
    optimized_search_pipeline(
                          60000, 2000,  
                          wandb_run_name=f"baseline-10-2",
                           embedding_model_name=f"sentence-transformers/all-MiniLM-L6-v2",
                          query_types = ["vector", "fts"],
                          k_values = [10],
                        table_name=tbl_name
    ...
    for epoch in epochs:
        ...
        tbl_name = f"gooaqnew_epoch_{epoch}_new"
        db = lancedb.connect("db")
        if tbl_name not in db:
            print("creating new table")
            ingest(0, 87600, table_name=tbl_name, model=f"ayushexel/embed-all-MiniLM-L6-v2-squad-{epoch}-epochs")
            tbl = db[tbl_name]
            tbl.create_fts_index("context", use_tantivy=False)
            tbl.create_index(index_type="IVF_HNSW_SQ", vector_column_name="embedding")
        tbl = db[tbl_name]
        ...
        optimized_search_pipeline(
                              60000, 2000,
                              wandb_run_name=f"emb-all-MiniLM-{epoch}-epochs-5",
                               embedding_model_name=f"ayushexel/embed-all-MiniLM-L6-v2-squad-{epoch}-epochs",
                              query_types = ["vector", "fts"],
                              k_values = [5],
                            table_name=tbl_name
                             )
        optimized_search_pipeline(
                              60000, 2000,
                              wandb_run_name=f"emb-all-MiniLM-{epoch}-epochs-10",
                               embedding_model_name=f"ayushexel/embed-all-MiniLM-L6-v2-squad-{epoch}-epochs",
                              query_types = ["vector", "fts"],
                              k_values = [10],
                            table_name=tbl_name
