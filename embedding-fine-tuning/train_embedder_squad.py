...
def main(num_epochs=1):
    ...
    full_dataset = load_dataset("rajpurkar/squad", split="train").select(range(50000))
    ...
    # Ensure correct column order for MultipleNegativesRankingLoss
    hard_train_dataset = hard_train_dataset.select_columns(['question', 'context', 'negative'])
    ...
    hard_eval_dataset = mine_hard_negatives(
        ...
        anchor_column_name="question",
        positive_column_name="context",
        corpus=full_dataset["context"],
        num_negatives=1,
        batch_size=512,
        include_positives=True,
        output_format="triplet",
    )
    hard_eval_dataset = hard_eval_dataset.select_columns(['question', 'context', 'negative_1'])
    logging.info(hard_eval_dataset)
    evaluator = TripletEvaluator(
    anchors=hard_eval_dataset["question"],
    positives=hard_eval_dataset["context"],
    ...
    run_name = f"emb-{short_model_name}-squad-{num_epochs}-epochs"
    ...
    model=model,
    args=args,
    train_dataset=hard_train_dataset,  # Use the hard negative dataset instead of original
    eval_dataset=hard_eval_dataset,    # Use the hard negative eval dataset
    loss=loss,
    evaluator=evaluator,
    )
    trainer.train()
